...
0.4.0 在 VS Code 左侧 Activity Bar 增加 smart2stupid 独立入口。点击图标即可固定打开 Claude 实时执行视图，不再需要先找命令或占用编辑区；需要更宽空间时可点“在编辑区展开”。本轮 Claude token、缓存输入、费用和分模型明细也会直接显示。
...
4. Codex 调用 **npm run delegate** 后，左侧 Activity Bar 的 smart2stupid 执行视图自动打开。
5. Claude 完成后 Codex 只读审核；未通过时最多自动修正三轮。

Activity Bar 图标和状态栏的 **smart2stupid** 都可随时重新打开当前执行视图。

## 面板操作

- **停止 Claude**：终止当前执行并标记为 cancelled，不会自动重跑。
- **在 Claude Code 中打开**：按 session ID 打开官方 Claude Code 会话。
- **在编辑区展开**：保留左侧入口，同时在编辑区打开宽版执行面板。
...
- **smart2stupid.autoOpenExecutionPanel**：检测到委派任务时自动打开左侧执行视图，默认开启。
